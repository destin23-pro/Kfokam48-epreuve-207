...
Le schéma est celui que produisent les migrations Flyway `V1` à `V5` (`backend/src/main/resources/db/migration/`). Toute évolution du schéma passe par une nouvelle migration **et** une mise à jour de ce diagramme dans le même commit.
...
erDiagram
    ...
    EXERCICES ||--o| RELECTURES : "est relu par (0 ou 1, RG7)"
    ...
    RELECTURES {
        BIGSERIAL id PK
        BIGINT exercice_id FK "NOT NULL, UNIQUE (RG7)"
...
| `CHECK source IN ('ETUDIANT','FORMATEUR')` | presences | V2 (colonne), V5 (check) | RG11 |
| `UNIQUE (session_id, etudiant_id)` | exercices | V5 | RG9 : un exercice par étudiant et par session (409) |
| `UNIQUE (exercice_id)` | relectures | V5 | RG7 : un seul relecteur par exercice |
| `note` nullable, `CHECK note BETWEEN 0 AND 20` | relectures | V5 | RG3. La relecture est créée `EN_ATTENTE` sans note au dépôt (EF5) |
...
- Un exercice a 0 ou 1 relecture : 0 tant qu'aucun relecteur n'est disponible (RG13).
